...
--select "idClasses", ass.*, mam."fAssetClassSet"(ass.id, "idClasses") from pl."vPlayListResolved" plr, (select max(ia."idItems") as idi, a.id FROM pl."tItemAttributes" ia, mam."tAssets" a where ia."sKey"='asset' and "nValue" = a.id group by a.id) ass where idi=plr.id order by ass.id
...
CREATE OR REPLACE FUNCTION mam."fPersonRemove"(id integer) RETURNS int_bool AS
$$
DECLARE
	stRetVal int_bool;
	nAssetsQty integer;
BEGIN
	SELECT count(*) INTO nAssetsQty FROM mam."vAssetsPersons" ap WHERE ap."idPersons"=id;
