...
----------------------------------- logs."tSCR"
	...
	CREATE TABLE logs."tGame"
		(
			id serial NOT NULL,
			"idPlaylistItems" integer NOT NULL,
			"idAssets" integer NOT NULL,
			"dtStartPlanned" timestamp with time zone NOT NULL,
			"bIsActual" boolean NOT NULL,
			CONSTRAINT "tGame_pkey" PRIMARY KEY (id)
		)
		WITH (
			OIDS=FALSE
		);
