--CREATE DATABASE "replica" WITH OWNER = pgsql ENCODING = 'UTF8';
/*
	CREATE OR REPLACE FUNCTION plpgsql_call_handler() RETURNS language_handler AS '$libdir/plpgsql', 'plpgsql_call_handler'
		LANGUAGE 'c' VOLATILE;
	ALTER FUNCTION plpgsql_call_handler() OWNER TO pgsql;

	CREATE OR REPLACE FUNCTION plpgsql_validator(oid) RETURNS void AS '$libdir/plpgsql', 'plpgsql_validator'
		LANGUAGE 'c' VOLATILE;
...
CREATE USER replica_init LOGIN CREATEROLE SUPERUSER ENCRYPTED PASSWORD  '' INHERIT VALID UNTIL 'infinity';
...
------------------------------------ tINP
	...
	CREATE OR REPLACE FUNCTION cast_to_xml(oINP tINP) RETURNS xml AS
		$$
			SELECT CASE WHEN oINP.id IS NULL THEN NULL ELSE xmlforest(oINP.id AS "nID", oINP."sName") END
		$$
		LANGUAGE sql VOLATILE;
		CREATE CAST (tINP AS xml) WITH FUNCTION cast_to_xml(tINP);

	CREATE OR REPLACE FUNCTION cast_from_xml_tINP(oXML xml) RETURNS tINP AS
		$$
			SELECT CASE WHEN oXML IS NULL THEN NULL ELSE ROW((xpath('/*/nID/text()', oXML))[1]::text::bigint, (xpath('/*/sName/text()', oXML))[1]::name)::tINP END
		$$
		LANGUAGE sql VOLATILE;
		CREATE CAST (xml as tINP) WITH FUNCTION cast_from_xml_tINP(xml);
------------------------------------
...
CREATE OR REPLACE FUNCTION "fMicrosecondsElapsed"(dt timestamp) RETURNS int AS
	$$
	DECLARE
		nRetVal integer;
	BEGIN
