...
----------------------------------- media."tFileAttributes"
	...
	CREATE OR REPLACE FUNCTION media."fFileAttributeSet"(
			idfiles bigint,
		...
		DECLARE
			...
			idRegisteredTables integer;
		BEGIN
			stRetVal := hk."fRegisteredTableGet"('media', 'tDates');
			IF NOT stRetVal."bValue" THEN
				RAISE EXCEPTION 'CANNOT GET A REGISTERED TABLE ENTRY FOR media."tDates"';
			END IF;
			idRegisteredTables := stRetVal."nValue";
			stRetVal := media."fFileAttributeGet"(idFiles, idRegisteredTables, sKey);
			IF NOT stRetVal."bValue" THEN
				stRetVal := media."fDictionaryAdd"(dtValue);
				IF NOT stRetVal."bValue" THEN
					RAISE EXCEPTION 'CANNOT ADD A DICTIONARY VALUE [%]', dtValue;
				END IF;
				stRetVal := media."fFileAttributeAdd"(idFiles, idRegisteredTables, sKey, stRetVal."nValue");
			ELSE
				UPDATE media."tDates" SET "dt"=dtValue WHERE id=stRetVal."nValue";
				stRetVal."bValue" := true;
			END IF;
			RETURN stRetVal;
		END;
	...
	CREATE OR REPLACE FUNCTION media."fDictionary"(
			etarget target,
			dtvalue timestamp with time zone)
		RETURNS int_bool AS
		$BODY$
		...
		BEGIN
			stTable."sSchema":='media';
			stTable."sName":='tDates';
			aColumns := '{{dt,0}}';
			aColumns[1][2] := quote_literal(dtValue); --чтобы не морочиться с экранированием
			IF 'add' = eTarget THEN 
				stRetVal := "fTableAdd"(stTable, aColumns, true);
				IF NOT stRetVal."bValue" AND stRetVal."nValue" IS NULL THEN
					stRetVal := "fTableGet"(stTable, aColumns);
	...
	CREATE OR REPLACE FUNCTION media."fDictionaryGet"(dtvalue timestamp with time zone)
		RETURNS int_bool AS
		$BODY$
		DECLARE
			stRetVal int_bool;
		BEGIN
			stRetVal := media."fDictionary"('get', dtValue);
			RETURN stRetVal;
		END;
		$BODY$
		LANGUAGE plpgsql VOLATILE;

	CREATE OR REPLACE FUNCTION media."fDictionaryAdd"(dtvalue timestamp with time zone)
		RETURNS int_bool AS
		$BODY$
		DECLARE
			stRetVal int_bool;
		BEGIN
			stRetVal := media."fDictionary"('add', dtValue);
			RETURN stRetVal;
		END;
		$BODY$
		LANGUAGE plpgsql VOLATILE;
	...
	CREATE OR REPLACE FUNCTION media."fDictionaryGarbageCollector"()
		RETURNS int_bool AS
		$BODY$
		DECLARE
			idRegisteredTables integer;
		...
		BEGIN
			...
			END IF;
			idRegisteredTables := stRetVal."nValue";
			DELETE FROM media."tDates" WHERE id NOT IN (SELECT d.id FROM media."tFileAttributes" ta, media."tDates" d WHERE idRegisteredTables = ta."idRegisteredTables" AND d.id = ta."nValue");
